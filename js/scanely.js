...
function setPageMetadata(profile) {
  const title = `${profile.title} | Scanely`;
  const description = profile.bio || "Open this Scanely social profile page.";
...
function createPlatformIcon(iconPath, label, className) {
  if (iconPath === "globe") {
    const wrapper = createElement("span", className);
    wrapper.setAttribute("aria-hidden", "true");
    wrapper.innerHTML = `<span class="link-card__glyph link-card__glyph--svg">${createIconMarkup("globe")}</span>`;
    return wrapper;
  }
...
function renderActionRow(profile) {
  const actions = createElement("div", "action-row action-row--utility");
